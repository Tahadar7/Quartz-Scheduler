...
public class UpdateHttpJobRequestValidator : AbstractValidator<UpdateHttpJobRequest>
{
    public UpdateHttpJobRequestValidator()
    {
        RuleFor(x => x.JobName)
        ...
        RuleFor(x => x.Headers)
            ...
            .When(x => !string.IsNullOrWhiteSpace(x.Headers));      // Only validate if Headers is not null
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(json))    // if null or whitespace, consider it valid (optional field)
        {
            return true;
        }
